...
const emit = defineEmits<{ (e: 'transfer', machine: Machine): void }>();

const filter = ref('all');
const filteredMachines = computed(() =>
  filter.value === 'all' ? props.machines : props.machines.filter((m) => m.status === filter.value),
);
...
<template>
  <section class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <el-table-column label="操作" width="120">
        <template #default="{ row }">
          <el-button
            ...
            type="primary"
            plain
            :disabled="row.status !== MACHINE_IDLE"
    ...
    <p class="mt-2 text-xs text-slate-400">仅空闲农机可发起转场；转场中或作业中的农机按钮自动禁用。</p>
  </section>
</template>
