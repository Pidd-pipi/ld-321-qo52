...
| POST | /dashboard/tasks/:id/dispatch | 一键派单（推荐空闲农机与驾驶员，在途农机返回 409 拒绝） | - |
| GET | /dashboard/reports/work/export | 作业报表导出信息 | - |
| POST | /transfers | 发起转场（仅空闲农机，登记目标地块/预计到达/申请人） | - |
| POST | /transfers/:id/arrive | 到达确认（更新所属地块为目标地块，恢复可派） | - |
| POST | /transfers/:id/cancel | 申请人取消（农机回到原地块，记录失败原因） | - |
| GET | /transfers?machineCode= | 转场记录查询，可按农机编号筛选 | - |
| GET | /ws | WebSocket 实时轨迹推送 | - |
| GET | /healthz | 健康检查（DB + Redis） | - |

### 农机转场办理流程

地块之间借调农机通过转场单办理，避免管理员直接改所属地块导致在途农机仍被派单：

1. **发起**：仅状态为「空闲」的农机可发起，需登记目标地块、预计到达时间和申请人；发起后农机变为「转场中」，所属地块暂不改变。
2. **在途**：同一台农机同时只允许一张未结束（在途）转场单；在途期间派单接口返回 `409`，前端调度按钮同步禁用。
3. **到达确认**：确认后所属地块更新为目标地块，农机恢复「空闲/可派」，转场单置为「已到达」。
4. **取消**：申请人可取消在途转场单，农机回到原地块并恢复可派，单据置为「已取消」并记录失败原因。
5. **记录**：转场记录支持按农机查看，列表展示起点地块、目标地块、状态和失败原因。
